...
class LifeLone_MNIST(object):
    ...
    def Calculate_Elbo2(self,testX,vaeLoss):
        # domain 1
        ...
        sum = 0
        for i in range(myN):
            sum = sum + self.sess.run(vaeLoss,feed_dict={self.inputs:testX[i*self.batch_size:(i+1)*self.batch_size]})
        sum = sum / myN
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            self.saver.restore(sess, 'models/TeacherStudent_Complex32')
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            myVAELoss = self.Give_ElboNetwork()
            elboArr = []
            ...
            for taskIndex in range(taskCount):
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    elbo = self.Calculate_Elbo2(self.mnist_test_x,myVAELoss)
                    elboArr.append(elbo)

                lossArr1 = np.array(elboArr).astype('str')
                f = open("results/LongTask_ELBOs.txt", "w", encoding="utf-8")
                ...
                f.close()
                '''
                self.saver.save(self.sess, "models/TeacherStudent_Complex32")
