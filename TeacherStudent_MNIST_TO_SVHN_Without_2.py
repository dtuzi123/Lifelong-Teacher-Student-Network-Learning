...
class LifeLone_MNIST(object):
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            testLabels = np.concatenate((self.mnist_label, self.svhn_label), axis=0)
            ...
            batch_y = testY[0:self.batch_size]
            batch_images = testX[0:self.batch_size]
            ...
            real = np.reshape(batch_images, (-1, 32, 32, 3))
            ims("results/" + "MnistToSVHN_reco" + str(0) + ".jpg", merge2(y_RPR1[:64], [8, 8]))
            ims("results/" + "MnistToSVHN_gan" + str(0) + ".jpg", merge2(g_output[:64], [8, 8]))
            ims("results/" + "MnistToSVHN_real" + str(0) + ".jpg", merge2(real[:64], [8, 8]))
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            #self.saver.restore(sess, 'models/TeacherStudent_MNIST_TO_SVHN_Without')
            ...
            myThirdName = "results/my11.txt"
            lossArr1 = str(mnistAccuracy)
            f = open(myThirdName, "w", encoding="utf-8")
            f.writelines(lossArr1)
            f.flush()
            f.close()
            ...
            for taskIndex in range(currentTask):
                ...
                myThirdName = "results/MnistToSVHN_Without_MNISTAccuracy_task" + str(taskIndex) + ".txt"
            ...
            #self.saver.save(self.sess, "models/TeacherStudent_MNIST_TO_SVHN_Without")
