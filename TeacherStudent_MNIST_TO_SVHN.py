...
class LifeLone_MNIST(object):
    ...
    def build_model(self):
        ...
        label_logits2 = Image_classifier(self.inputs, "auxilary_classifier", reuse=False)
        self.auxliaryClassifier_loss = tf.reduce_mean(
        ...
        self.codeMini_classloss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=code_predLogit, labels=self.labels))
        ...
        CodeClassifier_vars = [var for var in T_vars if var.name.startswith('code_classifier')]

        AuxilaryClassifier_vars = [var for var in T_vars if var.name.startswith('auxilary_classifier')]
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            self.code_classifier_optim_postive = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(self.codeMini_classloss, var_list=CodeClassifier_vars)
            self.code_classifier_optim_negative = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(-self.codeMini_classloss, var_list=encoder_vars1 + generator1_vars)

            self.auxilary_classifier_optim_negative = tf.train.AdamOptimizer(self.learning_rate, beta1=self.beta1) \
                .minimize(-self.auxliaryClassifier_loss, var_list=AuxilaryClassifier_vars)
    ...
    def predict_auxilary(self):
        # define the classifier
        label_logits = Image_classifier(self.inputs, "auxilary_classifier", reuse=True)
        label_softmax = tf.nn.softmax(label_logits)
        predictions = tf.argmax(label_softmax, 1, name="predictions")
        return predictions
    ...
    def Calculate_Discrepancy(self,testX):
        totalN = np.shape(testX)[0]
        myN = int(totalN / self.batch_size)
        myPrediction = self.predict()
        myPrediction2 = self.predict_auxilary()
        totalPredictions = []
        totalPredictions2 = []
        myCount = 0
        for i in range(myN):
            my1 = testX[self.batch_size * i:self.batch_size * (i + 1)]
            predictions = self.sess.run(myPrediction, feed_dict={self.inputs: my1})
            predictions2 = self.sess.run(myPrediction2, feed_dict={self.inputs: my1})
            for k in range(self.batch_size):
                totalPredictions.append(predictions[k])
                totalPredictions2.append(predictions2[k])

        totalPredictions = np.array(totalPredictions)
        totalPredictions2 = np.array(totalPredictions2)
        trueCount = 0
        for k in range(np.shape(totalPredictions)[0]):
            if totalPredictions[k] == totalPredictions2[k]:
                trueCount = trueCount + 1
        accuracy = (float)(trueCount / np.shape(totalPredictions)[0])
        error = 1-accuracy
        return error
    ...
    def Calculate_accuracy_true(self,testX,testY):
        #testX = self.mnist_test_x
        totalN = np.shape(testX)[0]
        myN = int(totalN / self.batch_size)
        myPrediction = self.predict_auxilary()
        totalPredictions = []
        myCount = 0
        for i in range(myN):
            my1 = testX[self.batch_size * i:self.batch_size * (i + 1)]
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            testX = np.concatenate((self.mnist_train_x, self.svhn_train_x), axis=0)
            testY = np.concatenate((self.mnist_train_y, self.svhn_train_y), axis=0)
            ...
            ims("results/" + "mnistToSVHN_gan" + str(0) + ".png", merge2(g_outputs[:36], [6, 6]))

            real1 = testX[0:self.batch_size]
            ims("results/" + "mnistToSVHN_real" + str(0) + ".png", merge2(real1[:36], [6, 6]))

            my11 = self.sess.run(self.output1, feed_dict={self.inputs: testX[0:self.batch_size]})
            ims("results/" + "mnistToSVHN_reco" + str(0) + ".png", merge2(my11[:36], [6, 6]))
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            sourArr = []
            targetArr = []
            auxilaryTrainingSet = self.mnist_train_x
            auxilaryTrainingLabel = self.mnist_label
            trueResults = []
            discArr = []
            for taskIndex in range(taskCount):
                ...
                if taskIndex == 1:
                    auxilaryTrainingSet = np.concatenate((self.mnist_train_x,self.svhn_train_x),axis=0)
                    auxilaryTrainingLabel = np.concatenate((self.mnist_label,self.svhn_label),axis=0)
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    n_examples2 = np.shape(auxilaryTrainingLabel)[0]
                    index2 = [i for i in range(n_examples2)]
                    random.shuffle(index2)
                    auxilaryTrainingLabel = auxilaryTrainingLabel[index2]
                    auxilaryTrainingSet = auxilaryTrainingSet[index2]
                    ...
                    for idx in range(start_batch_id, self.num_batches):
                        ...
                        auxilaryX = auxilaryTrainingSet[idx * self.batch_size:(idx + 1) * self.batch_size]
                        auxilaryY = auxilaryTrainingLabel[idx * self.batch_size:(idx + 1) * self.batch_size]
                        ...
                        _ ,c_class= self.sess.run([self.mini_classifier_optim, self.mini_classLoss],
                                                   feed_dict={self.inputs: batch_images, self.y: batch_y,self.labels:batch_labels,self.z:batch_z})

                        # Update network by classification
                        _ = self.sess.run([self.code_classifier_optim_postive],
                                          feed_dict={self.inputs: batch_images, self.y: batch_y,
                                                     self.labels: batch_labels,self.z:batch_z})
                        _ = self.sess.run([self.code_classifier_optim_negative],
                                          feed_dict={self.inputs: batch_images, self.y: batch_y,
                                                     self.labels: batch_labels,self.z:batch_z})

                        _ = self.sess.run(self.auxilary_classifier_optim_negative,
                                          feed_dict={self.inputs: auxilaryX,self.y: batch_y,self.z:batch_z,
                                                     self.labels: auxilaryY})
                    ...
                    if isFirstStage == True:
                        ims("results/" + "MNIST_TO_SVHN_first" + str(0) + ".png", merge2(g_output[:36], [6, 6]))

                    mnistTargetRisk = 1- self.Calculate_accuracy(self.mnist_test_x, self.mnist_label_test)
                    mnistTrueRisk = 1- self.Calculate_accuracy_true(self.mnist_test_x, self.mnist_label_test)

                    if taskIndex == 0:
                        targetArr.append(mnistTargetRisk)
                        trueResults.append(mnistTrueRisk)
                    if taskIndex == 1:
                        svhnTargetRisk = 1 - self.Calculate_accuracy(self.svhn_test_x, self.svhn_label_test)
                        targetRisk = svhnTargetRisk+mnistTargetRisk
                        targetArr.append(targetRisk)

                        svhnTrueRisk = 1 - self.Calculate_accuracy_true(self.svhn_test_x, self.svhn_label_test)
                        trueAccumulated = svhnTrueRisk+mnistTrueRisk
                        trueResults.append(trueAccumulated)

                    sourArr.append(c_class)
                    if taskIndex == 0:
                        error1 = self.Calculate_Discrepancy(self.mnist_test_x)
                        error2 = self.Calculate_Discrepancy(self.mnist_train_x)
                        disc1 = np.abs(error1 - error2)
                        discArr.append(disc1)
                    if taskIndex == 1:
                        error1 = self.Calculate_Discrepancy(self.mnist_test_x)
                        error2 = self.Calculate_Discrepancy(self.mnist_train_x)
                        disc1 = np.abs(error1 - error2)

                        data1 = np.concatenate((self.mnist_test_x,self.svhn_test_x),axis=0)
                        data2 = dataX[0:np.shape(data1)[0]]

                        error1_ = self.Calculate_Discrepancy(data1)
                        error2_ = self.Calculate_Discrepancy(data2)
                        disc1_ = np.abs(error1_ - error2_)
                        discArr.append(disc1+disc1_)

            myName = 'results/TeacherStudent_Bound_Target.txt'
            targetArr = np.array(targetArr).astype('str')
            f = open(myName, "w", encoding="utf-8")
            for i in range(np.shape(targetArr)[0]):
                f.writelines(targetArr[i])
                f.writelines('\n')
            f.flush()
            f.close()

            myName = 'results/TeacherStudent_TrueResults.txt'
            trueResults = np.array(trueResults).astype('str')
            f = open(myName, "w", encoding="utf-8")
            for i in range(np.shape(trueResults)[0]):
                f.writelines(trueResults[i])
                f.writelines('\n')
            f.flush()
            f.close()

            sourArr = np.array(sourArr).astype('str')
            myName = 'results/TeacherStudent_Bound_Source.txt'
            f = open(myName, "w", encoding="utf-8")
            for i in range(np.shape(sourArr)[0]):
                f.writelines(sourArr[i])
                f.writelines('\n')
            f.flush()
            f.close()

            discArr = np.array(discArr).astype('str')
            myName = 'results/TeacherStudent_Bound_Disc.txt'
            f = open(myName, "w", encoding="utf-8")
            for i in range(np.shape(discArr)[0]):
                f.writelines(discArr[i])
                f.writelines('\n')
            f.flush()
            f.close()
