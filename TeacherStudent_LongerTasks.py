...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.InverseFashionTrainX,self.InverseFashionTrainLabels = Give_InverseFashion()
        self.InverseFashionTrainY = np.zeros((np.shape(self.InverseFashionTrainX)[0], 4))
        self.InverseFashionTrainY[:,3] = 1
        cc1 = 0
    ...
    def build_model(self):
        ...
        self.output2 = reco2
        self.GAN_output = G1
        self.recoError = reconstruction_loss1
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            self.saver.restore(sess, 'models/TeacherStudent_LongerTasks3')

            myPredict = self.predict()

            myIndex = 2

            testX = np.concatenate((self.mnist_train_x,self.mnistFashion_train_x,self.thirdX,self.InverseFashionTrainX),axis=0)
            testY = np.concatenate((self.mnist_train_y,self.mnistFashion_train_y,self.thirdY,self.InverseFashionTrainY),axis=0)
            ...
            ims("results/" + "mnistToFashion_gan" + str(0) + ".png", merge(g_outputs[:64], [8, 8]))

            real1 = testX[0:self.batch_size]
            my11 = np.reshape(real1,(-1,28,28,1))
            ims("results/" + "longerTasks_real" + str(0) + ".png", merge(my11[:64], [8, 8]))

            my11 = self.sess.run(self.output1, feed_dict={self.inputs: testX[0:self.batch_size]})
            my11 = np.reshape(my11,(-1,28,28,1))
            ims("results/" + "long可爱颂017"
                             "erTasks_reco" + str(0) + ".png", merge(my11[:64], [8, 8]))
    ...
    def train(self):
        ...
        currentTask = 4
        myReco = tf.placeholder(tf.float32, [self.batch_size,28,28,1])
        recoError = tf.reduce_mean(tf.reduce_sum(tf.square(myReco - self.inputs), [1, 2, 3]))
        with tf.Session() as sess:
            self.sess = sess
            sess.run(tf.global_variables_initializer())
            self.saver = tf.train.Saver()
            ...
            for taskIndex in range(currentTask):
                ...
                if taskIndex == 1:
                    currentTrainX = self.mnistFashion_train_x
                    currentTrainY = self.mnistFashion_train_y
                elif taskIndex == 2:
                    currentTrainX = self.thirdX
                    currentTrainY = self.thirdY
                elif taskIndex == 3:
                    currentTrainX = self.InverseFashionTrainX
                    currentTrainY = self.InverseFashionTrainY
                ...
                if taskIndex == 0:
                    ...
                    dataX = np.concatenate((currentTrainX, oldX), axis=0)
                    dataY = np.concatenate((currentTrainY, oldY), axis=0)
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    #ims("results/" + "MNIST" + str(epoch) + ".png", merge(y_RPR1[:64], [8, 8]))
                    #ims("results/" + "GAN" + str(epoch) + ".png", merge(g_output[:64], [8, 8]))

                    my2 = 0
                    totalCount = int(np.shape(self.mnist_test_x)[0]/self.batch_size)
                    for i in range(totalCount):
                        batch_y = self.mnist_test_y[i*self.batch_size:(i+1)*self.batch_size]

                        my1 = self.sess.run(
                            self.recoError,
                            feed_dict={self.inputs: self.mnist_test_x[i*self.batch_size:(i+1)*self.batch_size]})
                        my2 = my2 + my1
                    my2 = my2 / int(np.shape(self.mnist_test_x)[0]/self.batch_size)
                    recoArray.append(my2)

                    if epoch == 1:
                        b= 0

                recoArray = np.array(recoArray).astype('str')
                f = open("results/longer_reco_task"+str(taskIndex)+".txt", "w", encoding="utf-8")
                for i in range(np.shape(recoArray)[0]):
                    f.writelines(recoArray[i])
                    f.writelines('\n')
                f.flush()
                f.close()

                #self.saver.save(self.sess, "models/TeacherStudent_LongerTasks" + str(currentTask))
