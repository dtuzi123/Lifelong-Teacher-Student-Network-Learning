    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                dataX = np.concatenate((self.mnist_train_x,self.svhn_train_x),axis=0)
                dataY = np.concatenate((self.mnist_train_y,self.svhn_train_y),axis=0)
                labelsY = np.concatenate((self.mnist_label,self.svhn_label),axis=0)
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    mnistAccuracy = self.Calculate_accuracy(self.mnist_test_x, self.mnist_label_test)
                    mnistFashionAccuracy = self.Calculate_accuracy(self.svhn_test_x, self.svhn_label_test)
                    myArr.append(mnistAccuracy + mnistFashionAccuracy)
                    #mnistAccuracy_list.append(mnistAccuracy)
                    #mnistFashionAccuracy_list.append(mnistFashionAccuracy)

            myName = 'results/TeacherStudent_MS_bounds2.txt'
            lossArr1 = np.array(myArr).astype('str')
